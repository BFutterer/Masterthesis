/* map
 * This file contains all functions that are necessary for the
 *  - addGeoJson             draw the isochrones at the map
 *  - getBoundaryURL         build the URL to request the boundary of a city from Nominatim
 *  - inspect                send the request to Nominatim and display the citynames of the response in the dropdown menu
 *  - drawCityBoundary       draw the coosen city onto the map
 *  - deleteBoundaryLayer    delete boundary layer from the map
 *  - handleEnter            capture undesired enter events to avoid errors
 *  - clearAllFromMap        clear the map and all settings
 *  - getLocation            get the coordinates where the user clicked at the map
 *  - HSVtoRGB               convert HSV color values to RGB color values
 *  - toPaddedHexString      add 0 in front of hexadecimal numbers
 *  - generateIsochrones     compute the isochrones
 *
 *  L = Leaflet
 *  $ = jQuery
 */
...
function addGeoJson(json) {
  if (json.features){
      if (json.features[json.features.length - 1].type == "FeatureCollection"){ //check if the last element (alwas element for contour 1) has the type FeatureCollection
         converted = { "features" : [], "type" : "FeatureCollection" };         //create a new object
         json.features.forEach(function(x) {                                    //loop through json
           if (x.type == "FeatureCollection"){                                  //check if current element has the type FeatureCollection
             x.features.forEach(function(y) {                                   //loop through x
               y.properties = x.properties;                                     //copy the properties of the feature Collection to the single features
               converted.features.push(y);                                      //add the current elementto the object
             })
           }else if(x.type == "Feature"){                                       //check if the current element has the type Feature
             y={};                                                              //create a new object
             y.properties = x.properties;                                       //copy the properties of the feature Collection to the single features
             y.type = x.type;                                                   //copy the type of the feature Collection to the single features
             y.geometry = x.geometry;                                           //copy the geometry of the feature Collection to the single features
             converted.features.push(y);                                        //add the current elementto the object
           }
         });
         json = converted;                                                      //set json to converted
  ...
  geojson = L.geoJson(json, {
    ...
    onEachFeature: function(feature, layer) {
      var tooltip = layer.bindTooltip(feature.properties.contour + ' min', { sticky: true });	            //tooltip = Kurzinfo, defines what will be shown in case of a mouse-over (x min)    //sticky: true is needed to display the tooltip always next to the mouse
      tooltips.push(tooltip);                                                   //.push adds the object to an array
      tooltip.addTo(map);
    }
  });
...
function inspect(evt) {
  ...
  select.innerHTML = "<option value=100>Loading...</option>";
...
//capture undesired enter events to avoid errors
...
function clearAllFromMap(){                                                     //https://stackoverflow.com/questions/28646317/how-to-remove-all-layers-and-features-from-map answer of iH8
  ...
  position="";
  startLocation=null;
  $("#selectedCity_value").html("<option value=100>choose correct city</option>");
}
...
function getLocation(e){
  console.log("e");
  console.log(e);
  if (e.originalEvent instanceof MouseEvent){                                   //check if the event (originalEvent) is a mouse event
    ...
    console.log("other event: Enter key was pressed");                          //handleEnter function löschen
}
}
...
function toPaddedHexString(num, len) {                                          //add 0 in front of hexadecimal numbers
    str = num.toString(16);                                                     //https://stackoverflow.com/questions/9909038/formatting-hexadecimal-number-in-javascript answer of Nilesh
...
function generateIsochrones() {
  ...
  for(i=1; i <= localminutes; i++) {
    var color = HSVtoRGB((96/255)*(localminutes-i)/(localminutes), 1, 1);	                //first color is red [((96/255)*(1-1)/(minutes-1), 1, 1) = 0], last color is green [((96/255)*(20-1)/(20-1), 1, 1) = 0.37]
    ...
    contour[i - 1] = {"time": i,"color": toPaddedHexString(color.r, 2)+toPaddedHexString(color.g, 2)+toPaddedHexString(color.b, 2)};	//{"time":20,"color":"#abcdef"}		contour i-1 = i, because computer starts counting by 0, but minutes starts couning by 1
  }
  ...
  $.getJSON(url,function computeIsochrones(isochrones){
    ...
    //clear the tooltips
    tooltips.forEach(function (tooltip) {															          //tooltips are in an array -> forEach (like a loop)
  ...
  map.addLayer(marker);
  marker.bindPopup("<b>Name:</b><br />"+localsetId.toString()+"<br /><b>Starting point:</b><br />" + "lat: " + (Math.round(position.lat * 10000) / 10000) +", long: " + (Math.round(position.lng * 10000) / 10000));	   //display the coordinates of the clicked point in the popup window      Math.round rounds just to the nearest integer, therefore value*10000/10000
}
